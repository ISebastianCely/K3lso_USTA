...
class Teleop:
    def __init__(self):
        ...
        self.actions_voice = rospy.Subscriber('actions2',String,self.voice_callback)
...
CTRL-C to quit
        ...
        try:
            ...
            while not rospy.is_shutdown():
                ...
                if key in self.velocityBindings.keys():
                    #se toma un valor según la tecla
    ...
    def voice_callback(self,mensaje):
        print(mensaje.data)
        if mensaje.data in self.velocityBindings.keys():
            x = self.velocityBindings[mensaje.data][0]
            y = self.velocityBindings[mensaje.data][1]
            z = self.velocityBindings[mensaje.data][2]
            th = self.velocityBindings[mensaje.data][3]
            twist = Twist()
            twist.linear.x = x *self.speed
            twist.linear.y = y * self.speed
            twist.linear.z = z * self.speed
