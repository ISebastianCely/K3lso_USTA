...
class DxlDriver():
    ...
    def __init__(self, port, baudrate, dxl_info, initial_fake_pose, only_monitor, fake_mode):
        ...
        if not self.fake_mode:
            ...
            ####### MODIFY PID - TEST LEDS #########
            for motor_name in self.dxl_info:
                m_id = self.dxl_info[motor_name]['id']
                dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(
                                                    self.portHandler, m_id, LED, 1)
                if dxl_comm_result != COMM_SUCCESS or dxl_error != 0:
                    self.error('Couldn\'n set LEDs {}'.format(m_id)); return

            # for motor_name in self.dxl_info:
            #     m_id = self.dxl_info[motor_name]['id']
            #     dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(
            #                                         self.portHandler, m_id, P_GAIN, 200)
            #     if dxl_comm_result != COMM_SUCCESS or dxl_error != 0:
            #         self.error('Couldn\'n set P Gain {}'.format(m_id)); return

            # self.packetHandler.write1ByteTxRx(self.portHandler, 11, 80, 6)  # P
            # self.packetHandler.write1ByteTxRx(self.portHandler, 11, 82, 1)  # I
            # self.packetHandler.write1ByteTxRx(self.portHandler, 11, 84, 200)  # D
